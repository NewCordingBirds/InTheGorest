...
void CCamera::Init_ProjBuffer()
{
	...
	tBuffer.Usage = D3D11_USAGE_DYNAMIC;
	tBuffer.ByteWidth = sizeof(Buffer_Eye);
	tBuffer.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	tBuffer.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	FAILED_CHECK_RETURN(
		m_pDevice->GetDevice()->CreateBuffer(&tBuffer, NULL, &m_pEyeBuffer), );
}


void CCamera::Init_EyeBuffer()
{
	D3D11_BUFFER_DESC tBuffer;
	ZeroMemory(&tBuffer, sizeof(D3D11_BUFFER_DESC));
...
void CCamera::Invalidate_Eye()
{
	D3D11_MAPPED_SUBRESOURCE tSubreResource;
	m_pDeviceContext->Map(m_pEyeBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &tSubreResource);

	Buffer_Eye* pEye = (Buffer_Eye*)tSubreResource.pData;
	m_vEye = D3DXVECTOR3(pEye->m_vEye);

	m_pDeviceContext->Unmap(m_pEyeBuffer, 0);
	m_pDeviceContext->VSSetConstantBuffers(VS_SLOT_CAMERA_POSITION, 1, &m_pEyeBuffer);
}
