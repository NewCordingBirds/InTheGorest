								...
								const wstring& _wstrKey)
{
	...
	case SHADER_TYPE_TEXTURE :	pResource = CTextureShader::Create(_pDevice, false);	break;
	case SHADER_TYPE_UI:		pResource = CTextureShader::Create(_pDevice, true);	break;
								...
								vector<string> _vecAniName /*= vector<string>()*/)
{
	...
	switch (_eModelType)
	{
	case CResourceMgr::MODEL_TYPE_DYNAMIC:	pResource = CDynamicModel::Create(_pDevice, _pPath, _vecAniName); break;
	case CResourceMgr::MODEL_TYPE_STATIC :	pResource = CStaticModel::Create(_pDevice, _pPath);	break;
